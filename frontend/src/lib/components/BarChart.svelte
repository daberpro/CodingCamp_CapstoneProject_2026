<script>
	...
	let gridWidth = $derived(Math.max(640, series.length * 88));
</script>
...
<section class="my-8 overflow-hidden rounded-3xl bg-white p-5 shadow-sm sm:p-8">
	<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
		<div>
			...
			<p class="text-lg text-slate-700">Perbandingan sesuai rentang tanggal</p>
		</div>
	...
	{:else}
		<div class="overflow-x-auto pb-2">
			<div class="grid h-72 items-end gap-3 border-b border-dashed border-slate-200 px-1 sm:gap-4" style={`width: ${gridWidth}px; grid-template-columns: repeat(${series.length}, minmax(0, 1fr))`}>
				{#each series as item}
					<div class="flex min-w-0 flex-col items-center justify-end gap-2">
						...
						<div class="w-full truncate text-center text-sm">{item.day}</div>
					</div>
				{/each}
			</div>
		</div>
